export const metadata = {
  title: 'Webhooks',
  description:
    'In this guide, we will look at how to register and consume webhooks to integrate your app with Appmint.',
}

# Webhooks

In this guide, we will look at how to register and consume webhooks to integrate your app with Appmint. With webhooks, your app can know when something happens in Appmint, such as someone sending a message or adding a contact. {{ className: 'lead' }}

## Registering webhooks

To register a new webhook, you need to have a URL in your app that Appmint can call. You can configure a new webhook from the Appmint dashboard under [API settings](#). Give your webhook a name, pick the [events](#event-types) you want to listen for, and add your URL.

Now, whenever something of interest happens in your app, a webhook is fired off by Appmint. In the next section, we'll look at how to consume webhooks.

## Consuming webhooks

When your app receives a webhook request from Appmint, check the `type` attribute to see what event caused it. The first part of the event type will tell you the payload type, e.g., a conversation, message, etc.
    ...
    ```json {{ 'title': 'Example payload' }}
    {
      ...
      "payload": {
        ...
        "contact": {
          ...
          "avatar_url": "https://assets.Appmint.chat/avatars/kevin.jpg",
...
```php
$signature = $request['headers']['x-Appmint-signature'];
...
If your generated signature matches the `x-Appmint-signature` header, you can be sure that the request was truly coming from Appmint. It's essential to keep your secret webhook key safe — otherwise, you can no longer be sure that a given webhook was sent by Appmint. Don't commit your secret webhook key to GitHub!
